...
为解决这一局限，我们联合北京第二外国语学院搭建了专用视听翻译评测数据集，配套高质量语料集与一套以自动化评测为核心的完整评测框架。该基准可实现可扩展、可复现的模型综合测评，同时辅以随机人工抽检，用于甄别字幕中人物语气、文化适配度、观影体验等细微文本特征。同时我们正在与北二外一起制定一个针对影视字幕翻译的评价标准和数据集，后续将进一步开源该benchmark，以及在该benchmark上更新模型指标。
